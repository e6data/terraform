resource "google_compute_network" "network" {
  name                    = "${var.env}-${var.workspace_name}-network"
  auto_create_subnetworks = false
}

resource "google_compute_subnetwork" "subnetwork" {
  name          = "${var.env}-${var.workspace_name}-subnetwork"
  ip_cidr_range = var.gke_subnet_ip_cidr_range
  region        = var.gcp_region
  network       = google_compute_network.network.name

  secondary_ip_range {
    range_name    = var.gke_cluster_secondary_range_name
    ip_cidr_range = var.gke_cluster_ipv4_cidr_block
  }
...
resource "google_compute_router" "router" {
  name    = "${var.env}-${var.workspace_name}-router"
  region  = var.gcp_region
  network = google_compute_network.network.name
}

resource "google_compute_router_nat" "nat" {
  name                               = "${var.env}-${var.workspace_name}-nat"
