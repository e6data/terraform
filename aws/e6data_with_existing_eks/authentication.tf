...
module "e6data_configmap_and_api" {
  ...
  count  = contains(["API", "API_AND_CONFIG_MAP"], data.aws_eks_cluster.target_eks.access_config[0].authentication_mode) ? 1 : 0
...
resource "kubernetes_config_map_v1_data" "aws_auth_update" {
  ...
  count = data.aws_eks_cluster.target_eks.access_config[0].authentication_mode == "CONFIGMAP" ? 1 : 0
